"""
Credit: https://github.com/nhartland/KL-divergence-estimators/blob/master/src/knn_divergence.py
"""
...
def skl_estimator(s1, s2, k=1):
    ...
    s1_neighbourhood = NearestNeighbors(n_neighbors=k + 1, radius=10).fit(s1)
    s2_neighbourhood = NearestNeighbors(n_neighbors=k, radius=10).fit(s2)
...
def skl_estimator_efficient(s1, s2, k=1):
    ...
    s1_neighbourhood = NearestNeighbors(
        n_neighbors=k + 1, radius=radius, algorithm="kd_tree"
    ).fit(s1)
    s2_neighbourhood = NearestNeighbors(
        n_neighbors=k, radius=radius, algorithm="kd_tree"
    ).fit(s2)
